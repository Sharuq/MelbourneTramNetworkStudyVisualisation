# server.R for manipulation and plotting
...
shinyServer(function(input, output) {
  ...
  combinedPlotting<-function(data){
    ...
    thm=hc_theme_handdrawn()
    
    h = highchart() %>% 
      hc_yAxis_multiples(
        list(lineWidth = 3, lineColor="#ec0030", title=list(text="Frequency")),
        list(lineWidth = 3, lineColor='#400024', title=list(text="Average_load"))
      ) %>% 
...
output$caption <- renderText({
    paste("Average Load per Route During ", input$TimeP, "Period")
  })

output$fcaption <- renderText({
    paste("Frequency per Route During ", input$TimePeriod, "Period ", input$DayType )
  })  

output$scaption <- renderText({
    paste("Combined per Route During ", input$TimePer, "Period ")
  })  
